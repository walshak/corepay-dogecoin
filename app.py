...
def create_wallet():
    ...
    w = Wallet.create(name, keys=passphrase, network='dogecoin_testnet', db_uri=db_uri, owner=owner)
...
def get_balance():
    ...
    # scan for uxto's
    w.scan(network='dogecoin_testnet')
    
    return jsonify({'balance': w.balance(network='dogecoin_testnet')})
...
def get_address():
    ...
    w = Wallet(wallet_name,db_uri=db_uri)
    # get the wallet key
    key = w.get_key(network='dogecoin_testnet')
...
def send_to():
    ...
    #open wallet
    w = Wallet(wallet_name,db_uri=db_uri)
    t = t = w.send_to(reciever_address, amount ,network='dogecoin_testnet',offline=True)
